...
void CheckForOpen(){
    ...
    Print("Function \"", __FUNCTION__, "\" called.");
...
bool CheckForClose(){
    ...
    for (int i = 0; i < OrdersTotal(); i++){ // Scan for trading orders from old to new
        ...
        if ((OrderMagicNumber()!=g_MAGIC_NUMBER) || (OrderSymbol()!=Symbol())) continue;
        if (OrderType() == OP_BUY){
